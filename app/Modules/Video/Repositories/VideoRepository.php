...
class VideoRepository implements VideoInterface
{
    ...
    public function getTrendingVideo($limit = null, $filter = [], $sort = ['by' => 'id', 'sort' => 'DESC'], $status = [0, 1]){
            ...
            return Video::where('video_update','>=', $date)->orderBy('total_views','DESC')->paginate($limit ? $limit : env('DEF_PAGE_LIMIT', 9999)); 
   }
    ...
    {
        $result = Video::when(array_keys($filter, true), function ($query) use ($filter) {
        })->where(['display_block_section' => $block_id, 'genre_id' => $genre_id])->orderBy('id', $sort['sort'])->paginate($limit ? $limit : env('DEF_PAGE_LIMIT', 9999));
