...
<div class="card">
    <div class="card-body d-sm-flex align-items-sm-center justify-content-sm-between flex-sm-wrap">
        <a href="{{ route('employment.create') }}" class="btn bg-teal-400"><i class="icon-plus-circle2"></i> Add Employee</a>
        <div>
             <a href="{{ route('employment.indexArchive') }}" class="btn bg-warning mr-1"><i class="icon-folder-download3 mr-2"></i> Archived Employee</a>
    ...
    <div class="card">
        <div class="card-img-actions mx-1 mt-1 text-center">
            ...
            <div class="card-img-actions-overlay card-img">
               ...
               @endphp
               <input type="hidden" value="{{ $UserInfo->parent_id ?? '' }}" id="user_parent_id_{{$value->id}}" />
                ...
                @else
                    <a data-toggle="modal" data-target="#modal_theme_success" class="ml-1 btn btn-outline bg-white text-white border-white border-2 btn-icon rounded-round employer_user_access" emp_id="{{$value->id}}" data-popup="tooltip" data-placement="bottom" data-original-title="User Access"><i class="icon-user-plus"></i></a>
                @endif
                @if($value->is_parent_link == '1')
                    <a data-toggle="modal" data-target="#modal_parent_link" class="ml-1 btn btn-outline bg-white text-white border-white border-2 btn-icon rounded-round user_parent_link" empId="{{$value->id}}" data-placement="bottom" data-popup="tooltip" data-placement="top" data-original-title="Link With Parent"><i class="icon-user-check"></i></a>
                @else
                    <a data-toggle="modal" data-target="#modal_parent_link" class="ml-1 btn btn-outline bg-white text-white border-white border-2 btn-icon rounded-round user_parent_link" empId="{{$value->id}}" data-placement="bottom" data-popup="tooltip" data-placement="top" data-original-title="Link With Parent"><i class="icon-user-plus"></i></a>
                @endif
           </div>
       </div>

       <div class="card-body text-center">
        <h6 class="font-weight-semibold mb-0"><a href="javascript:void(0)" class="text-warning">{{ $value->first_name .' '. $value->middle_name.' '. $value->last_name }}</a></h6>
        <span class="d-block text-muted">{{ $value->designation->dropvalue }}</span>

        <ul class="list-inline list-inline-condensed mt-3 mb-0">
            <li class="list-inline-item">
                @if($value->status == '1')
                <a data-toggle="modal" data-target="#modal_theme_warning_status" class="btn btn-outline bg-success btn-icon text-success border-success border-2 rounded-round status_employee" employee_id="{{$value->id}}" data-popup="tooltip" data-placement="bottom" data-original-title="Move To Archive">
                    <i class="icon-basket"></i>
                </a>
                @else
                <a data-toggle="modal" data-target="#modal_theme_warning_status" class="btn btn-outline bg-danger btn-icon text-danger border-danger border-2 rounded-round status_employee" employee_id="{{$value->id}}" data-popup="tooltip" data-placement="bottom" data-original-title="In-Active Employer">
            ...
            <li class="list-inline-item"><a href="{{route('employment.edit',$value->id)}}" class="btn btn-outline bg-info btn-icon text-info border-info border-2 rounded-round" data-popup="tooltip" data-placement="bottom" data-original-title="Edit Employee">
...
<div class="ribbon-container">
    <div class="ribbon {{$color}}">
        <a class="text-light" href="" data-popup="tooltip" data-original-title="Employee Status" data-placement="bottom">{{$status}}</a>
    </div>
</div>
</div>
</div>
            ...
            <div class="modal-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-9">
                        ...
                        {!! Form::textarea('archive_reason', $value = null, ['id'=>'archive_reason','placeholder'=>'Enter Reason for Archive','class'=>'form-control','required'=>'required']) !!}
                    </div>
                </div>
                <div class="text-center">
